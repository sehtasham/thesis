...
expected_delay <- function(D, v, p_o, f_l, f_e, t_o, lambda) {
    mew_l <- task_execution_delay(D, v, f_l)
    mew_e <- task_execution_delay(D, v, f_e)
    ...
    Tw_l <- task_expected_waiting_time_device(D, v, f_l, p_o, lambda_l)
    Tw_e <- task_expected_waiting_time_edge(D, v, f_e, p_o, lambda_e)
