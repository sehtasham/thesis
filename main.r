...
P_t <- p_i
G <- c(0.5, 1, 1.5) * (-100) # Channel Gain different states
h <- G[2]
...
R_uav <- upload_data_rate_device_uav(W_i, p_i, noise_power,
                                alpha_0, L_uav, L_i)
...
h_i <- 127 + (30 * log10(L))
theta_i <- 0.5
R_node <- upload_data_rate_device_node(W, sigma_2, h_i, P_i, theta_i)
